...
## Locked Research Picks (Anthony, 2026-07-24)

These resolve the 6 open Research Directives + OQ1 in 03-RESEARCH.md. Anthony chose each from the researcher's surfaced options; they are now locked decisions with the same authority as D-01..D-18.

- **D-19 (canvas dependency, OQ1 approved):** Add `@napi-rs/canvas@1.0.2` so pdfjs-dist / `unpdf.renderPageAsImage` can rasterize an image-only PDF page to a bitmap (D-07). Prebuilt N-API binary, ABI-stable (no electron-rebuild), slopcheck OK. It is the only turnkey way to satisfy D-07.
- **D-20 (native-vs-scan gate, Directive 1 = Option B):** Docling-style layered gate. Per page, in order: (1) painted-bitmap coverage >= 0.75 of page area -> image-only; (2) else > 0.90 of glyph-show ops under text-render-mode 3 (invisible OCR overlay) -> image-only; (3) else >= 50 non-whitespace chars AND >= 1 embedded font -> native (belt-and-suspenders); (4) else image-only. Whole PDF is native iff >= 50% of pages are native. Char count is a soft tiebreaker only, never the sole gate. All thresholds are tunable via committed fixtures.
- **D-21 (multi-page, Directive 2 = Option A):** Single multi-image vision call carrying every page (native PDFs also send the full concatenated text), capped at 10 pages. Over the cap: send pages 1-3 + the last 2 and set a `truncated` flag Phase 6 surfaces. Cross-page reconciliation stays the Zod arithmetic check, not app-layer page-merging. One bill per file (V2-02 deferred).
- **D-22 (photo/scan confidence, Directive 3 = Option B):** Second-pass cross-call agreement check, scoped to image-only docs only. Both calls at temperature 0; a numeric/string mismatch on a key field (total, subtotal, tax, invoice date, invoice number) is an automatic low-confidence flag (flag-and-keep, D-12). Native PDFs skip the second call (verbatim-text grounding already covers them). Behind a config flag so it can be disabled if latency ever matters.
- **D-23 (prompt design, Directive 4 = adopt recommended verbatim):** Use the researcher's recommended prompt as the starting prompt, kept in one testable `src/main/parse/prompt.ts` constant. Text-before-image ordering; image declared ground truth, any transcription declared a noisy reference; "return null if absent, never invent"; only `vendor` and `total` are non-null-required, everything else nullable; money returned as the raw printed string (the Zod gate coerces to integer cents locally so the model never does math). Wording is tunable.
- **D-24 (cache schema, Directive 5 = 5a-A + 5b-A):** `migration0003` creates `parsed_results` STRICT, keyed on `file_hash` (SHA-256) PRIMARY KEY. Per-field confidence stored as a JSON blob column (`field_confidence TEXT`). Store `raw_response TEXT` (nullable) for audit/offline re-validation. Money as INTEGER cents (`subtotal_cents`/`tax_cents` nullable, `total_cents` NOT NULL). Include `model`, `route`, `page_count`, `base_url_host` (host only, never the key), `validation_flags` (JSON), `parsed_at` (ISO), and `schema_version` so a prompt/schema bump can deliberately force re-parse while a model switch alone never invalidates (D-14). Follow the recommended DDL in 03-RESEARCH.md Directive 5.
- **D-25 (client details, Directive 6 = defaults):** `new OpenAI({ apiKey, baseURL, maxRetries: 3, timeout: 120000 })`. Structured-output fallback ladder per call, chosen from the model's known capabilities: strict `json_schema` -> `json_object` + schema-in-prompt -> plain-prompt JSON. ALWAYS re-validate the returned text with the local Zod schema (authoritative); on Zod failure do one repair retry, then flag-and-keep the file as a parse failure (D-15). Vision classification per D-02: metadata-first (`architecture.input_modalities` includes `image`) -> curated vision-family regex -> unbadged -> D-01 confirm gate.
- **D-26 (parse trigger + progress IPC, OQ2/OQ3):** Auto-parse (D-13) runs as a separate `parse:parse-batch(loadedFiles)` channel the renderer fires on the loaded set after the scan, NOT inside the scan IPC call, for cleaner per-file isolation, progress, cancellation, and retry (D-15). Progress streams via a `parse:progress` main->renderer broadcast (`webContents.send`) mirroring the existing `theme:changed` pattern. Exact channel/type names are planner discretion within the `src/shared/ipc-contract.ts` conventions.

---

*Phase: 3-AI Client and Parse Pipeline*
*Context gathered: 2026-07-24*
*Research picks locked: 2026-07-24*
