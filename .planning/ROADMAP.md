...
- [x] 01-01-PLAN.md - Scaffold, hardened shell, native rebuild (Windows), and test harness

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 01-02-PLAN.md - Typed IPC contract, Zod schemas, and sandbox-safe preload bridge
- [x] 01-03-PLAN.md - Magnet Group brand theme: vendored tokens, local fonts, shadcn
- [x] 01-04-PLAN.md - SQLite persistence with migrations and the safeStorage secret store
...
- [x] 01-06-PLAN.md - Branded app shell, three screens, and the Secret store health round trip
...
- [x] 02-02-PLAN.md — Duplicate catch: already-posted files excluded-by-default with a one-click override; within-scan byte-identical copies collapse

**Wave 3** *(blocked on Wave 2)*

- [x] 02-03-PLAN.md — Not-ready skip: cloud-sync placeholders and partial writes detected by metadata (never downloaded), flagged and surfaced for re-scan
...
- [x] 03-03-PLAN.md - Deterministic validation gate (money->cents, dates->ISO, subtotal+tax=total) + deterministic-weighted per-field confidence (PARSE-04)
- [x] 03-04-PLAN.md - Native-vs-scan routing gate (Docling-style), unpdf text + pdfjs/canvas page render, HEIC->sharp image prep (PARSE-01/02)
- [x] 03-05-PLAN.md - Vision field extraction: D-23 prompt + text-before-image call, structured-output fallback ladder, local Zod re-validate + repair retry (PARSE-03)
- [x] 03-06-PLAN.md - Persistence: migration0003 parsed_results STRICT table + hash-keyed cache CRUD (PARSE-05 storage)
